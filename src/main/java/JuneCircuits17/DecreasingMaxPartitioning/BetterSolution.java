...
public class BetterSolution {
    ...
    public void solve() throws IOException, InputMismatchException {
        ...
        while (testCases-- != 0) {
            ...
            long currentCorrectOrder = 1;
            int currentWrongOrder = 0;
            int currMax = arr[n - 1];
            for (int i = n - 2; i >= 0; i--) {
                if (arr[i] < currMax) {
                    currentWrongOrder++;
                    if (currentCorrectOrder != 0) {
                        ans = (ans * currentCorrectOrder) % mod;
                        currentCorrectOrder = 0;
                    }
                    continue;
                } else {
                    currMax = arr[i];
                    if (currentCorrectOrder != 0)
                        currentCorrectOrder = (currentCorrectOrder * 2) % mod;
                    else
                        currentCorrectOrder = 1;
                    if (currentWrongOrder != 0) {
                        ans = (ans * (currentWrongOrder + 2)) % mod;
                        currentWrongOrder = 0;
                    }
