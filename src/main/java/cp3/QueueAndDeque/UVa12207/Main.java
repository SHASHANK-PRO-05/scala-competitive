package cp3.QueueAndDeque.UVa12207;
...
public class Main {
    ...
    public void solve() throws IOException, InputMismatchException {
        ...
        while (n != 0 && c != 0) {
            int current = 1;
            Deque<Integer> expedite = new LinkedList<>();
            Deque<Integer> checked = new LinkedList<>();
            while (c-- != 0) {
                switch (readString()) {
                    case "E":
                        int element = readInt();
                        if (expedite.contains(element)) {
                            int top = expedite.peek();
                            if (element == top) {
                                continue;
                            }
                            while (expedite.peek() != element) {
                                expedite.add(expedite.poll());
                            }
                            expedite.poll();
                            while (expedite.peek() != top) {
                                expedite.add(expedite.poll());
                            }
                            expedite.addFirst(element);
                    ...
                    case "N":
                        int printThis;
                        if (!expedite.isEmpty()) {
                            printThis = expedite.poll();
                        } else if (current <= n) {
                            while (checked.contains(current) && current <= n) {
                                current++;
                            }
                            if (current <= n) {
                                printThis = current;
                            } else {
                                printThis = checked.poll();
                            }
                        } else {
                            printThis = checked.poll();
                        }
